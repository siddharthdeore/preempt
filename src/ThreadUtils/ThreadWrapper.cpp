...
void ThreadWrapper::setPriority(int policy = SCHED_RR, int priority = 20)
{
    ...
    if (pthread_setschedparam(this->native_handle(), policy, &_sch_params))
    {
        ...
        std::cerr << "Failed to set thread scheduling, try with sudo : " << std::strerror(errno) << std::endl;
    }
    else
    {
...
void ThreadWrapper::setAffinity(int core)
{
    ...
    int ret = pthread_setaffinity_np(this->native_handle(), sizeof(cpu_set_t), &_cpuset);
    if (ret != 0)
    {
        std::lock_guard<std::mutex> lock(_mtx);
...
int ThreadWrapper::getMaxPriority()
{
    return sched_get_priority_max(_policy);
}

int ThreadWrapper::getMinPriority()
{
    return sched_get_priority_min(_policy);
}
